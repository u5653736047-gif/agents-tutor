...
## 三-A、AI 学科矩阵（知识库命中链路，2026-08-31 补充）

作品主题为人工智能学科，知识库灌有四本教材（ml-zhouzhihua 机器学习/西瓜书、dl-d2l 动手学深度学习 PyTorch 2nd、dl-goodfellow 深度学习/花书、ai-russell 人工智能：现代方法）与课标。先行检索探针：12 个候选选题全部 5/5 命中（带 document+page 引用）。本矩阵验证「检索命中 → 素材稿 → 引用可追溯 → 产物」的完整知识链路。

### 教案（引用可追溯性为核心断言：正文带「来源：document，页码」）

| # | 课题（对应教材） | 结果 | review | 引用标记数 | 产物校验（docx） | 耗时 |
|---|---|---|---|---|---|---|
| K1 卷积神经网络（dl-d2l/花书/西瓜书/AIMA 四书同引） | ✅ completed | pass | 全文逐条 | 63 段落 / 3,540 字符 / validate 0 错 | 155s |
| K2 决策树（ml-zhouzhihua） | ✅ completed | pass | 23 处 | 64 段落 / 3,509 字符 / validate 0 错 | 166s |
| K3 强化学习初步（ai-russell） | ✅ completed | pass | 21 处 | 62 段落 / 3,745 字符 / validate 0 错 | 158s |
| K4 过拟合与正则化（dl-goodfellow） | ✅ completed | pass | 34 处 | 101 段落 / 4,462 字符 / validate 0 错 | 227s |

K1 抽样（正文原文）：「理解 CNN 的定义与设计动机：稀疏交互、参数共享、等变表示（来源：dl-goodfellow，p324-328）」「掌握卷积层、填充、步幅、汇聚层等基本组件（来源：dl-d2l，p235、246-249）」——**教材+页码级引用**，可追溯断言达标。

### 课件（页数边界与主题路径断言沿用 §一设计）

| # | 课题（对应教材） | 结果 | review | 页数（请求/实际） | 主题 | 耗时 |
|---|---|---|---|---|---|---|
| K5 注意力机制与 Transformer（dl-d2l） | ✅ completed | pass | 16 / **16**（硬上界） | academic | 135s |
| K6 循环神经网络（dl-d2l） | ✅ completed | pass | 12 / 12 | edu | 100s |
| K7 梯度下降（花书/西瓜书） | ✅ completed | pass | 10 / **10**（硬下界） | edu（缺省路径） | 141s |
| K8 支持向量机（ml-zhouzhihua） | ✅ completed | pass | 缺省 12 / **12** | academic | 109s |

全部 pptx validate 0 错误；K8 封面截图核对：学术藏蓝渐变 + 哑金条 + Georgia 衬线英文标题。

### 观察项（内容级，非工作流缺陷）

- **课标素材过薄**：`course-a:guide` 仅 1 chunk，K1 collect 明确报「课标材料两次尝试均未命中」后按设计降级为纯教材素材——建议后续完整灌入课标文档（ingest 层改进，与本报告机制验证无关）；
- AI 矩阵 8 条与 §二通用矩阵合计 **16 条运行**：全部零审批、步骤 attempts=1（§三缺陷修复后无一次重试/冻结）、耗时 100~227s 稳定带宽。
...
2-A. AI 学科矩阵补验了知识库命中链路：四本教材全部被真实引用到教案正文（教材+页码级、可追溯），课件页数硬边界与主题路径在 AI 选题下同样精确命中；
