...
> ⚠️ 已冻结声明 2026-08-26 — 范围见 docs/SIX_FEATURES_COMPETITION_MAPPING.md，进度以本文件 + docs/TASKS_M3_CLOSE.md 为准；总清单 3.x 勾选以本文件出口检查为准
> ~~本文档是执行层清单，面向负责开发的 agent 使用。总清单（`TASK_BREAKDOWN_v2.md`）仍然是唯一的范围与进度权威来源；骨架清单（`docs/TASKS_STAGE_3_BRIDGE.md`）是施工框架的事实来源（契约字段、目录约定、组件结构）。~~ 本文档是执行层清单，面向负责开发的 agent 使用。~~总清单（`TASK_BREAKDOWN_v2.md`）仍然是唯一的范围与进度权威~~ → 范围见 `docs/SIX_FEATURES_COMPETITION_MAPPING.md`，骨架清单（`docs/TASKS_STAGE_3_BRIDGE.md`）是施工框架的事实来源（契约字段、目录约定、组件结构）。本文档只把「明确不做」各条拆解为可独立验收的原子任务，且**只写骨架未实现的内容**。
...
2. **勾选同步约定（已迁移 2026-08-26，不再回写冻结文件 `TASK_BREAKDOWN_v2.md`）**：每个任务完成后勾选本文档对应项；总清单阶段三
...
### [~] D6-T8 E2E 自动化验收（Playwright）— 静态交付(待Linux CI实测)
...
### [~] D6-T9 docker-compose 编排（延伸项，不属于 M3 出口）— 静态交付(待Linux CI实测)
...
> 以下逐项对齐总清单阶段三 3.x 各项；**勾选同步约定（已迁移 2026-08-26，不再回写冻结文件 `TASK_BREAKDOWN_v2.md`）**：细节清单全部任务勾选后，以本文件出口检查为准核对 3.x 各项——
> 每个 D 任务完成时已在本文档勾选对应子项，出口检查时逐项核对无遗漏。
...
- [x] 后端三项门禁 + 前端三项门禁全程无退化（每个任务完成时复核）；最终全量
  复核：后端 744→979 pytest + ruff + mypy strict 全绿（1053 行门禁已校准：E2E/Docker 待补测，基准 744→979，见 SIX_FEATURES 2026-08-26），前端 220 test + lint +
