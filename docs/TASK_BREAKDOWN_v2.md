...
- [ ] **（部分完成）** 1.1.3 实现协调者（Supervisor）模式
  ...
  - [x] 多结果按计划步骤一一归档，Supervisor 可统一汇总并对局部失败稳定降级。
...
- [x] 3.1.1 RESTful API 设计（M3 出口同步勾选，2026-08-05）
  - `POST /chat`：发送消息（返回流式响应）
  - `GET /sessions`：会话列表
  - `POST /sessions`：创建新会话
  - `GET /sessions/{id}/history`：历史消息
  - `POST /feedback`：用户反馈收集（D6-T1/D6-T2）
- [x] 3.1.2 流式通信（定案以 SSE 达成，M3 出口同步勾选）
  - Agent 思考过程实时推送（thinking → tool_call → response）（D1-T1 事件序列）
  - 多 Agent 协作进度可视化事件（D1-T1 tool_call/tool_result + D2-T2 面板）
  - 断线重连 + 消息补发机制（D1-T3 from_sequence 补发）
- [ ] 3.1.3 认证与限流（单独立项，未在本里程碑实施，M3 出口备注）
  - JWT Token 认证
  - 按用户限流（防止滥用）
  - 请求队列管理
...
- [x] 3.2.1 对话界面（M3 出口同步勾选）
  - 流式消息渲染（SSE / WebSocket 接收）（D1-T2）
  - Markdown + LaTeX（KaTeX）+ 代码高亮渲染（D3-T1 ~ D3-T3）
  - 多 Agent 回复的视觉区分（角色头像/标签）（骨架 W1-T1/T4 + D3-T4 引用）
- [x] 3.2.2 会话管理界面（M3 出口同步勾选）
  - 会话列表 + 搜索 + 归档（D4-T1/T7）
  - 历史对话回溯（骨架已达成）
  - 学习进度仪表盘（D6-T7 基础统计版）
- [x] 3.2.3 知识库管理界面（教师端）（M3 出口同步勾选；「知识条目浏览与编辑」以只读详情口径达成，见 D6-T6 备注）
  - 文档上传 + 解析状态展示（D6-T5/D6-T6）
  - 知识条目浏览与编辑（只读浏览口径；编辑依赖后端 chunk 端点，不在本期）
  - 检索效果测试工具（D6-T3/D6-T4）
- [x] 3.2.4 Agent 协作可视化（M3 出口同步勾选；「子代理并行进度条」待 core 1.1.3，以计划步骤进度达成，见 D2-T2 备注）
  - 实时展示当前活跃的 Agent 及其任务（D2-T2 活跃 Agent）
  - 工具调用过程透明化（可展开/折叠）（D2-T2）
  - 子代理并行执行进度条（待 core 1.1.3，以 current_step_index 计划进度为口径）

#### 3.3 前后端集成

- [x] 3.3.1 API 客户端封装（TypeScript SDK）（M3 出口同步勾选）
  - 类型安全的请求/响应定义（zod schema）（openapi-typescript 生成契约）
  - 自动重连 + 错误处理（D1-T3 重连、D2-T5 错误码、D4-T2 乐观回滚）
- [x] 3.3.2 流式渲染管线（M3 出口同步勾选）
  - WebSocket 消息 → 状态更新 → 增量 DOM 渲染（SSE → store → DOM，D1-T2）
  - Agent 状态机事件驱动 UI 变化（D2-T2）
- [x] 3.3.3 文件上传与多模态输入（M3 出口同步勾选；识别类高级能力待 core 多模态，见 D7-T3 备注）
  - 图片上传（手写公式识别）（上传/附件/渲染闭环 D7-T1~T3；公式识别待 core）
  - PDF 上传（学生作业）（D7-T1~T3）
  - 语音输入（可选）（未实施，可选深化项）
