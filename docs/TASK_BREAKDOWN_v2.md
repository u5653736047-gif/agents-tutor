...
- [ ] 修复知识来源、Citation 一致性和重复文档坐标问题
  （`Citation.source` 目前仍是服务器绝对路径，存在信息泄漏风险）。
- [x] 为状态 reducer、恢复流程、上下文裁剪和知识替换语义补充关键注释。
- [ ] 再次执行多代理 review、全量测试、静态检查和真实 DeepSeek 冒烟验证。
  2026-08-02 复核：全量测试 158 通过，ruff / mypy strict 干净；
  真实 DeepSeek 冒烟验证尚未执行。
- [ ] 创建加固提交并推送 `soldier`，核对远端提交哈希
  （本地领先 `origin/soldier` 9 个提交，尚未推送）。
