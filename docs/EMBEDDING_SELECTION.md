...
- **更换 provider 的约束**：不同 provider 的向量维度可能不同（哈希替身
  256 维、bge-small-zh-v1.5 为 512 维），切换后必须 `--force` 重建向量库
  ——向量索引加载旧库时会做维度校验并给出明确报错（`_load_all`），
  `_dot` 也有长度断言，不会静默截断计算（详见 vector_index.py 注释）。
